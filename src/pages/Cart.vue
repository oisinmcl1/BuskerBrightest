...
export default {
  ...
  methods: {
    ...
    async confirmCart() {
      try {
        ...
        for (const product of this.cart) {
          const { name, price, customerKey } = product;
          console.log(product);
          ...
          // Include the user's UID, product details, customer key, and UID in the request body
          const requestBody = {
            uid: user.uid, // Add the user's UID
            name: name,
            price: price,
            customerKey: customerKey // Include customerKey in the request body
          };
    ...
    async clearCart() {
      try {
        // Make a request to clear the cart (assuming there's a backend function for this)
